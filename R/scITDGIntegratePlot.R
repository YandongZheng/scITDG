...
#'
#' @examples
scITDGIntegratePlot <- function(exp.cur, row.dist = row.dist, k.num = 15,
                                time.points = time.points,
                                sample.ncell.use = sample.ncell.use,
                                integrate.cluster.list = NULL, cluster.order.list = NULL,
  ...
  if (show.trajectory) {
    ...
    if (!is.null(cluster.order.list)) {
      only_heatmap_name <- paste0(save.wd, tissue, "_scITDGPlot_NoTtrajectory_NoTerm_", save.type, ".pdf")
    } else {
      only_heatmap_name <- paste0(save.wd, tissue, "_scITDGPlot_NoTtrajectory_NoTerm_NoOrder_", save.type, ".pdf")
    }
  ...
  save(order.mat.df, file = paste0(save.wd, tissue, "_scITDG_Cluster_GeneName_", save.type, ".rdata"))
}
