...
#'
#' @examples
scITDGPlot <- function(exp.cur, row.dist = row.dist, k.num = 4, time.points = time.points,
                       sample.ncell.use = sample.ncell.use, cluster.order.list = NULL,
                       show.trajectory = TRUE, show.term = TRUE, OrgDb = org.Hs.eg.db,
                       save.wd = NULL, use_raster = TRUE, plot.height = NULL,
                       heatmap.cols = NULL) {
  if (is.null(save.wd)) {
    save.wd <- "./"
  }
  ...
  if (show.trajectory) {
    ...
    if (show.term) {
      ...
      pdf(paste0(save.wd, tissue, "_scITDGPlot.pdf"),
      ...
      line.subvp <- grid::viewport(x = 0.602, y = 0.493, width = 0.6, height = 0.985) # x越小越靠左边，y越小越靠下
      print(plot.ggplot, vp = line.subvp)

      col.bar.subvp <- grid::viewport(x = 0.157, y = 0.018, width = 0.275, height = 0.06)
      print(anno.bar.col, vp = col.bar.subvp)

      col.bar.subvp <- grid::viewport(x = 0.430, y = 0.018, width = 0.170, height = 0.06)
      print(anno.bar.col, vp = col.bar.subvp)
      ...
      pdf(paste0(save.wd, tissue, "_scITDGPlot_NoTerm.pdf"),
          width = 5, height = plot.height, bg = "transparent")
      draw(heatmap.plot, padding = unit(c(8, 2, 2, 65), "mm")) ## see right heatmap in following 底部、左侧、顶部和右

      line.subvp <- grid::viewport(x = 0.65, y = 0.493, width = 0.35, height = 0.985) # x越小越靠左边，y越小越靠下
      print(plot.ggplot, vp = line.subvp)

      col.bar.subvp <- grid::viewport(x = 0.245, y = 0.018, width = 0.429, height = 0.06)
      print(anno.bar.col, vp = col.bar.subvp)
    ...
    if (!is.null(cluster.order.list)) {
      only_heatmap_name <- paste0(save.wd, tissue, "_scITDGPlot_NoTtrajectory_NoTerm.pdf")
    } else {
      only_heatmap_name <- paste0(save.wd, tissue, "_scITDGPlot_NoTtrajectory_NoTerm_NoOrder.pdf")
    }
